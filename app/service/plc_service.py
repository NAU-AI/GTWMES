...
from service.PLC.snap7 import read_bool, read_int, read_real, write_alarms_data, read_uint, write_bool, write_int
from service.PLC.snap7 import plc_connect, plc_disconnect
from service.equipment_variables_service import (
...
class PlcService:
    def __init__(
        ...
        self.counting_equipment_service = counting_equipment_service or CountingEquipmentService()
        self.equipment_output_service = equipment_output_service or EquipmentOutputService()
    ...
    def _read_bool(self, plc, data):
        return read_bool(plc, int(data["db_address"]), data["offset_byte"], data["offset_bit"])
    ...
    def _write_bool(self, data, value):
        try:
            ...
            logging.info(
                f"Bool data successfully written"
            )
            return write_bool(plc, int(data["db_address"]), data["offset_byte"], data["offset_bit"], value)
        except Exception as e:
            logging.error(
